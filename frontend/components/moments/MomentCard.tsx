...
import MomentReviewControls, {
    type MomentReviewMetadata, type OnMomentReview,
} from './MomentReviewControls'
...
const statusVariant = (status: MomentReviewStatus | null): StatusChipVariant => {
        ...
        <article className="moment-card">
            ...
            {sample ? (
                <p className="moment-sample" title={sample.text}>
                    ...
                    {sample.count && sample.count > 1 ? <span className="moment-sample-count mono"> ×{sample.count.toLocaleString()}</span> : null}
                </p>
            ) : null}
            {clipUrl ? (
...
export type { MomentReviewMetadata }
