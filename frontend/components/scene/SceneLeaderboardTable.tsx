...
import type { SceneLeaderboardEntry } from '@/hooks/scene/useSceneLiveQueries'
...
type LeaderboardSortKey = 'rank' | 'name' | NumericColumnKey
...
interface LeaderboardColumn {
    ...
    render: (entry: SceneLeaderboardEntry) => ReactNode
}
...
const renderCreator = (entry: SceneLeaderboardEntry) => (
...
const numericColumn = (key: NumericColumnKey, label: string, digits = 0): LeaderboardColumn => ({
    key,
    label,
    align: 'end',
...
const sortValue = (row: SceneLeaderboardEntry, key: LeaderboardSortKey): string | number | null => (
    key === 'name'
...
const compareRows = (
    first: SceneLeaderboardEntry,
    second: SceneLeaderboardEntry,
...
const defaultSortDirection = (key: LeaderboardSortKey): SortDirection => (
    key === 'name' || key === 'rank' ? 'asc' : 'desc'
)

interface SceneLeaderboardTableProps {
    entries: SceneLeaderboardEntry[]
}

const SceneLeaderboardTable = ({ entries }: SceneLeaderboardTableProps) => {
    const { sort, dir, onSort } = useTableSort<LeaderboardSortKey>({
        initialKey: 'rank',
