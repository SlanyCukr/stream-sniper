...
import { expandThumbnailUrl } from './list/thumbnailUrl'
import ThumbImage from './list/ThumbImage'
import {
    formatDurationBetween, formatStreamTimestamp, formatTimeAgo,
} from '@/utils/dateUtils'
import type { StreamInfo } from '@/hooks/stream/list/useStreamsQuery'

interface StreamInfoCardProps {
    streamInfoData: StreamInfo
    /** optional export dropdown rendered in the page-actions slot next to the Twitch button */
...
const StreamInfoCard = ({
    streamInfoData, downloadMenu,
}: StreamInfoCardProps) => {
    const {
        title, displayName, messageCount, thumbnailUrl, nick, start, end,
    } = streamInfoData
    const twitchLink = nick ? `https://twitch.tv/${nick}` : ''
    const formattedStartTime = formatStreamTimestamp(start)
    const formattedEndTime = end == null ? 'Live' : formatStreamTimestamp(end)
    const timeAgo = formatTimeAgo(start)
    const duration = formatDurationBetween(start, end)

    const heroThumb = useMemo(() => expandThumbnailUrl(thumbnailUrl, 640, 360), [
        thumbnailUrl,
    ])
