...
import type { ReportMetric, ReportMoment, StreamReport } from '@/hooks/stream/report/useStreamReportQuery'

const percent = (value: number): string => `${formatDecimal(value * 100)}%`

type MetricKey = keyof StreamReport['metrics']
...
const DeltaChip = ({ deltaPct }: { deltaPct: number }) => {
...
export const hasReportContent = (data: StreamReport | null | undefined): data is StreamReport => {
...
interface ReportMetricTileData {
    key: MetricKey
    label: string
    phosphor?: boolean
    metric: ReportMetric
...
const ReportMetricGrid = ({ data }: { data: StreamReport }) => {
    const tiles: ReportMetricTileData[] = METRICS.flatMap(definition => {
...
const ReportHighlights = ({ data }: { data: StreamReport }) => {
    const moments: ReportMoment[] = data.topMoments || []
...
const StreamReportPresentation = ({ data }: { data: StreamReport }) => (
    <>
