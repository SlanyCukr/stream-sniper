...
import type { TimelineBucket, ViewerSample } from './useStreamTimelineQuery'

export const TIMELINE_WIDTH = 1000
export const MESSAGE_HEIGHT = 160
export const VIEWER_HEIGHT = 72

interface ViewerLanePoint {
    idxFrac: number
    viewers: number
    x: number
}
...
export interface ViewerLane {
    points: ViewerLanePoint[]
    maxViewers: number
    line: string
    area: string
}

export const buildViewerLane = (
    buckets: Pick<TimelineBucket, 't'>[],
    ...
    const y = (viewers: number) => VIEWER_HEIGHT - (viewers / maxViewers) * VIEWER_HEIGHT
    const line = points
...
export const getMarkerLeft = (buckets: Pick<TimelineBucket, 't'>[], timestamp: string): number => {
    const count = buckets.length
    if (count === 0) return 0
...
export const getNearestViewerCount = (
...
export const useTimelineGeometry = (
    ...
    const markerLeft = useCallback(
        (timestamp: string) => getMarkerLeft(buckets, timestamp),
        [buckets],
    )
    ...
    return {
        ...
        crosshairLeft: hovered
            // `hovered` truthy implies hoverIndex is not null, but TS can't link the two locals.
            ? `calc(2.6rem + ${((hoverIndex as number + 0.5) / bucketCount)} * (100% - 2.6rem))`
            : null,
    }
}
