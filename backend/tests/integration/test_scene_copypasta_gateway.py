"""PostgreSQL integration tests for the scene copypasta rollup (migration 0009).
...
from stream_sniper.database.gateways.content.stream_copypasta_stats_table_gateway import (
...
class TestStreamCopypastaSource:
    ...
    def test_source_applies_junk_and_bot_filters(self, copypasta_db):
        creator = create_test_creator(
            copypasta_db,
            {"nick": "c", "display_name": "C", "profile_image_url": "u", "twitch_id": 1},
        )
        stream = create_test_stream(
            copypasta_db, {"twitch_id": 11, "title": "S", "start": datetime(2024, 1, 1)}, creator
        )
...
class TestSceneCopypastasAggregate:
    ...
    def seeded(self, copypasta_db):
        ...
        s_recent_c1 = create_test_stream(
            copypasta_db, {"twitch_id": 21, "title": "R1", "start": now - timedelta(days=1)}, c1
        )
        s_recent_c2 = create_test_stream(
            copypasta_db, {"twitch_id": 22, "title": "R2", "start": now - timedelta(days=2)}, c2
        )
        s_old_c1 = create_test_stream(
            copypasta_db, {"twitch_id": 23, "title": "O1", "start": now - timedelta(days=40)}, c1
        )
