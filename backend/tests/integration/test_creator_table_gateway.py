"""
PostgreSQL integration tests for creator table gateway functions.
...
class TestCreatorTableGateway:
    ...
    def test_find_or_insert_creator_id_db_success(self, db_cursor, sample_creator_data):
    ...
    def test_find_or_insert_creator_id_db_conflict_handling(self, db_cursor, sample_creator_data):
        """Test that insert handles conflicts gracefully (ON CONFLICT DO NOTHING)."""
        # Insert creator first time
        first_id = find_or_insert_creator_id_db(
            sample_creator_data["nick"],
            sample_creator_data["display_name"],
            sample_creator_data["profile_image_url"],
            sample_creator_data["twitch_id"],
        ...
        second_id = find_or_insert_creator_id_db(
            sample_creator_data["nick"],
...
class TestCreatorTableGatewayWithMocks:
    ...
    def test_select_creator_id_db_with_mock(self, mock_connection_pool):
        ...
        assert result == 42
        mock_cursor.execute.assert_called_once_with("SELECT id FROM creator WHERE nick = %s", ("test_creator",))
    ...
    def test_find_or_insert_creator_id_db_with_mock(self, mock_connection_pool, sample_creator_data):
        """Test find_or_insert_creator_id_db with mocked database."""
        mock_pool, mock_connection, mock_cursor = mock_connection_pool
        mock_cursor.fetchone.return_value = (42,)

        result = find_or_insert_creator_id_db(
            sample_creator_data["nick"],
            sample_creator_data["display_name"],
            sample_creator_data["profile_image_url"],
            sample_creator_data["twitch_id"],
