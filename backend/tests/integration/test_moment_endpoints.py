"""PostgreSQL integration and HTTP tests for highlight queue and review mutations.
...
from stream_sniper.api.features.content.moment_endpoints import router
...
from stream_sniper.database.gateways.content.records import MomentQueueRow
from stream_sniper.database.gateways.content.stream_moment_table_gateway import (
    moment_exists_db,
...
def _client() -> TestClient:
    app = FastAPI()
    app.state.runtime = SimpleNamespace(cache=_miss_cache())
...
class TestMomentQueueEndpoint:
    @patch("stream_sniper.api.features.content.moment_endpoints.get_cache")
    @patch("stream_sniper.api.features.content.moment_endpoints.select_moment_queue_db")
...
class TestSetMomentReview:
    @patch("stream_sniper.api.features.content.moment_endpoints.invalidate_cache_pattern")
    @patch("stream_sniper.api.features.content.moment_endpoints.upsert_moment_review_db")
    @patch("stream_sniper.api.features.content.moment_endpoints.moment_exists_db")
    def test_admin_upsert_returns_status_and_invalidates(self, mock_exists, mock_upsert, mock_invalidate):
        mock_exists.return_value = True
        mock_upsert.return_value = "2024-01-15T21:00:00"

        response = _admin_client().put(
            "/streams/42/moments/2024-01-15T20:10:00/review",
        ...
        cache = mock_invalidate.call_args_list[0].args[0]
        mock_invalidate.assert_any_call(cache, "stream_timeline:*")
        mock_invalidate.assert_any_call(cache, "moments_queue:*")
        mock_invalidate.assert_any_call(cache, "stream_report:*")

    @patch("stream_sniper.api.features.content.moment_endpoints.invalidate_cache_pattern")
    @patch("stream_sniper.api.features.content.moment_endpoints.upsert_moment_review_db")
    @patch("stream_sniper.api.features.content.moment_endpoints.moment_exists_db")
    def test_missing_moment_404_no_write_no_invalidate(self, mock_exists, mock_upsert, mock_invalidate):
    ...
    @patch("stream_sniper.api.features.content.moment_endpoints.upsert_moment_review_db")
    @patch("stream_sniper.api.features.content.moment_endpoints.moment_exists_db")
    def test_clipped_requires_url_and_persists_metadata(self, mock_exists, mock_upsert, mock_invalidate):
        mock_exists.return_value = True
...
class TestClearMomentReview:
    @patch("stream_sniper.api.features.content.moment_endpoints.invalidate_cache_pattern")
    @patch("stream_sniper.api.features.content.moment_endpoints.delete_moment_review_db")
    @patch("stream_sniper.api.features.content.moment_endpoints.moment_exists_db")
    def test_admin_clear_returns_no_content_and_invalidates(self, mock_exists, mock_delete, mock_invalidate):
        mock_exists.return_value = True
        mock_delete.return_value = 1

        response = _admin_client().delete("/streams/42/moments/2024-01-15T20:10:00/review")
    ...
    def test_unauthenticated_rejected(self):
        response = _client().delete("/streams/42/moments/2024-01-15T20:10:00/review")
        assert response.status_code in (401, 403)

    def test_non_admin_forbidden(self):
        response = _user_client().delete("/streams/42/moments/2024-01-15T20:10:00/review")
        assert response.status_code == 403
...
class TestMomentQueueGateway:
    def test_pending_filter_creator_filter_total_and_status(self, queue_db):
        c1 = create_test_creator(
            queue_db, {"nick": "c1", "display_name": "C1", "profile_image_url": "x", "twitch_id": "1"}
        )
        c2 = create_test_creator(
            queue_db, {"nick": "c2", "display_name": "C2", "profile_image_url": "x", "twitch_id": "2"}
        )
        s1 = create_test_stream(queue_db, {"twitch_id": 101, "title": "S1"}, c1)
        s2 = create_test_stream(queue_db, {"twitch_id": 102, "title": "S2"}, c2)
    ...
    def test_pagination_limit_offset(self, queue_db):
        c1 = create_test_creator(
            queue_db, {"nick": "cc", "display_name": "CC", "profile_image_url": "x", "twitch_id": "9"}
        )
        s1 = create_test_stream(queue_db, {"twitch_id": 103, "title": "SP"}, c1)
    ...
    def test_moment_exists(self, queue_db):
        c1 = create_test_creator(
            queue_db, {"nick": "ce", "display_name": "CE", "profile_image_url": "x", "twitch_id": "7"}
        )
        s1 = create_test_stream(queue_db, {"twitch_id": 104, "title": "SE"}, c1)
        bucket = _insert_moment(queue_db, s1, 12, 8.0)
        queue_db.connection.commit()

        assert moment_exists_db(s1, bucket.strftime("%Y-%m-%dT%H:%M:%S")) is True
        assert moment_exists_db(s1, "2024-01-15T23:59:00") is False
