"""PostgreSQL integration tests for the analytics-expansion tables (migration 0008).
...
from stream_sniper.database.gateways.analytics.stream_chatter_stats_table_gateway import recompute_stream_rollup_db
from stream_sniper.database.gateways.analytics.stream_emote_stats_table_gateway import (
    select_creator_emotes_db,
    select_stream_emotes_db,
)
from stream_sniper.database.gateways.analytics.stream_metrics_table_gateway import select_stream_metrics_db
...
from stream_sniper.database.gateways.chat.emote_dictionary_table_gateway import (
...
from stream_sniper.database.gateways.content.moment_review_table_gateway import (
    delete_moment_review_db,
    upsert_moment_review_db,
)
from stream_sniper.database.gateways.content.records import MomentWriteRow
...
class TestEmoteDictionaryGateway:
    def test_seed_and_count(self, analytics_db):
        seed_emote_dictionary_db([("KEKW", "bttv", "5f9b1a2c3d4e5f6a7b8c9d0e"), ("Pog", "bttv", "aaaaaaaaaaaa")])
    ...
    def test_twitch_upsert_validates_provider_id(self, analytics_db):
        ...
        analytics_db.execute("SELECT name, provider_id FROM emote_dictionary WHERE source = 'twitch' ORDER BY name")
...
class TestStreamEmoteStatsGateway:
    def test_select_stream_and_creator_emotes(self, analytics_db):
        ...
        s1 = create_test_stream(analytics_db, {"twitch_id": 101, "title": "s1"}, creator_id)
        s2 = create_test_stream(analytics_db, {"twitch_id": 102, "title": "s2"}, creator_id)
...
class TestStreamTextRollups:
    def test_replace_writes_phrases_and_moments_atomically(self, analytics_db):
        creator_id = create_test_creator(analytics_db)
        stream_id = create_test_stream(analytics_db, {"twitch_id": 103, "title": "s1"}, creator_id)
        bucket = "2026-01-01T20:05:00"
    ...
    def test_replace_is_idempotent(self, analytics_db):
        creator_id = create_test_creator(analytics_db)
        stream_id = create_test_stream(analytics_db, {"twitch_id": 104, "title": "s1"}, creator_id)
        b = "2026-01-01T20:05:00"
        moment = MomentWriteRow(b, 0, 20, 2.0, 10.0, 5, None, None, None, None)
        replace_stream_text_rollups_db(stream_id, [("a", 3, 2)], [moment])
        replace_stream_text_rollups_db(stream_id, [("b", 4, 3)], [moment])
        prows = select_stream_phrases_db(stream_id, 25)
...
class TestMomentReviewGateway:
    def test_upsert_delete_roundtrip(self, analytics_db):
        creator_id = create_test_creator(analytics_db)
        stream_id = create_test_stream(analytics_db, {"twitch_id": 105, "title": "s1"}, creator_id)
        b = "2026-01-01T20:05:00"
        replace_stream_text_rollups_db(stream_id, [], [MomentWriteRow(b, 0, 20, 2.0, 10.0, 5, None, None, None, None)])
...
class TestCreatorOverlapGateway:
    ...
    def test_recompute_excludes_bots(self, overlap_seed, analytics_db):
        ...
        analytics_db.execute("UPDATE chatter SET is_bot = TRUE, bot_reason = 'known_bot' WHERE nick = 'chB'")
        analytics_db.connection.commit()
