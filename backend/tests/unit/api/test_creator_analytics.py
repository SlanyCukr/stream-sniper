...
from stream_sniper.api.features.creators.analytics_endpoints import router
...
class TestCreatorTrendsEndpoint:
    ...
    @patch("stream_sniper.application.creators.analytics_query.select_creator_metrics_series_db")
    def test_trends_success_ascending(self, mock_series, mock_get_cache):
        ...
        mock_series.return_value = [
            CreatorTrendRow(1, "Older Stream", "2024-01-10T20:00:00", 3600, 500, 8.33, 40, 30, 10),
            CreatorTrendRow(2, "Newer Stream", "2024-01-15T20:00:00", 7200, 1200, 10.0, 90, 20, 70),
        ]

        with TestClient(app) as client:
            response = client.get("/creators/5/trends")
    ...
    @patch("stream_sniper.api.features.creators.analytics_endpoints.get_cache")
    @patch("stream_sniper.application.creators.analytics_query.select_creator_metrics_series_db")
    def test_trends_model_survives_cache_round_trip(self, mock_series, mock_get_cache):
        cache = InProcessCache()
        mock_get_cache.return_value = cache
        mock_series.return_value = [
            CreatorTrendRow(1, "Cached Stream", "2024-01-10T20:00:00", 3600, 500, 8.33, 40, 30, 10)
        ]

        with TestClient(app) as client:
            first = client.get("/creators/5/trends")
            second = client.get("/creators/5/trends")
    ...
    def test_trends_null_duration_serializes_null(self, mock_series, mock_get_cache):
        ...
        mock_get_cache.return_value = _miss_cache()
        mock_series.return_value = [
            CreatorTrendRow(3, "Ongoing Stream", "2024-01-20T20:00:00", None, 100, 0.0, 20, 20, 0),
        ]
    ...
    def test_trends_custom_limit_forwarded(self, mock_series, mock_get_cache):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/trends?limit=50")
    ...
    def test_trends_limit_over_max_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/trends?limit=101")
    ...
    def test_trends_limit_below_min_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/trends?limit=0")
...
class TestCreatorSummaryEndpoint:
    ...
    @patch("stream_sniper.api.features.creators.analytics_endpoints.get_cache")
    @patch("stream_sniper.application.creators.analytics_query.select_creator_summary_db")
    def test_summary_maps_rollup_row(self, mock_summary, mock_get_cache):
        ...
        mock_get_cache.return_value = _miss_cache()
        mock_summary.return_value = CreatorSummaryRow(
            5,
            "alice",
            "Alice",
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/summary")
    ...
    def test_summary_missing_creator_404(self, mock_summary, mock_get_cache):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/404/summary")
...
class TestCreatorRegularsEndpoint:
    ...
    def test_regulars_attendance_rate_computed(self, mock_regulars, mock_count, mock_get_cache):
        ...
        mock_regulars.return_value = [
            CreatorRegularRow(42, "chatty_user", 6, "2024-01-01T20:00:00", "2024-01-15T20:00:00", 99, 1250),
            CreatorRegularRow(15, "regular_viewer", 3, "2024-01-02T20:00:00", "2024-01-14T20:00:00", 98, 640),
        ]

        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?min_streams=1")
    ...
    def test_regulars_zero_total_streams_no_zero_division(self, mock_regulars, mock_count, mock_get_cache):
        ...
        mock_regulars.return_value = [
            CreatorRegularRow(42, "chatty_user", 3, "2024-01-01T20:00:00", "2024-01-15T20:00:00", 99, 1250),
        ]

        with TestClient(app) as client:
            response = client.get("/creators/5/regulars")
    ...
    def test_regulars_sort_and_dir_forwarded(self, mock_regulars, mock_count, mock_get_cache):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?sort=last_seen&dir=asc&min_streams=4&limit=25")
    ...
    def test_regulars_include_bots_forwarded(self, mock_regulars, mock_count, mock_get_cache):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?include_bots=true")
    ...
    def test_regulars_bogus_sort_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?sort=bogus")
    ...
    def test_regulars_sql_injection_sort_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?sort=;DROP TABLE creator")
    ...
    def test_regulars_bad_dir_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?dir=sideways")
    ...
    def test_regulars_min_streams_out_of_range_422(self):
        ...
        with TestClient(app) as client:
            response = client.get("/creators/5/regulars?min_streams=0")
