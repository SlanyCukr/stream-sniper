"""DB-backed tests for select_stream_viewer_samples_db (session-anchor + windowing + tz).
...
from stream_sniper.database.stream_viewer_sample_table_gateway import select_stream_viewer_samples_db
...
def _tracked_streamer(cursor, creator_id, username="tracked_user"):
    cursor.execute(
        "INSERT INTO tracked_streamers (creator_id, twitch_username, display_name) VALUES (%s, %s, %s) "
        "RETURNING id",
...
def _stream(cursor, creator_id):
    return create_test_stream(
        cursor,
        {"twitch_id": "vsvod", "title": "vs", "start": _START, "end": _END},
        creator_id,
    )
