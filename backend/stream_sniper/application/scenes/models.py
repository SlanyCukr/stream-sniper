"""Canonical read models for scene analytics."""
...
class LiveStreamer(BaseModel):
    ...
    title: str | None = Field(None, description="Stream title at the latest sample, if known")
...
class LeaderboardEntry(BaseModel):
    ...
    hours_streamed: float | None = Field(None, description="Summed hours of closed streams in the window")
    total_messages: int = Field(..., description="Total messages across the window's streams")
    msgs_per_min: float | None = Field(None, description="Duration-weighted avg msgs/min; null when no rolled-up data")
    chatter_appearances: int = Field(
        ..., description="Summed per-stream unique chatters (double-counts across streams)"
    )
    peak_viewers: int | None = Field(None, description="Max live viewers in the window; null when no samples")
...
class Copypasta(BaseModel):
    ...
    first_seen: str | None = Field(None, description="Earliest send time (ISO 8601), if known")
    last_stream_start: str | None = Field(
