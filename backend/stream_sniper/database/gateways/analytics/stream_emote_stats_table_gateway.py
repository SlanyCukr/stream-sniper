...
def select_stream_emotes_db(
    ...
    return [TopEmoteRow(*row) for row in cursor.fetchall()]


@with_cursor
def select_creator_emotes_db(
    cursor: Cursor,
    creator_id: int,
    limit: int,
) -> list[CreatorEmoteRow]:
    ...
    return [CreatorEmoteRow(*row) for row in cursor.fetchall()]
