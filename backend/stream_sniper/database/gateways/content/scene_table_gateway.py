...
@with_cursor
def select_scene_leaderboard_db(
    cursor: Cursor,
    days: int,
) -> list[SceneLeaderboardRow]:
    ...
    return [SceneLeaderboardRow(*row) for row in cursor.fetchall()]


@with_cursor
def select_scene_peak_viewers_db(
    cursor: Cursor,
    days: int,
) -> list[ScenePeakViewerRow]:
    ...
    return [ScenePeakViewerRow(*row) for row in cursor.fetchall()]
